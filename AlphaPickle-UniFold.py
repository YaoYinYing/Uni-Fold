...
class AlphaFoldMetaData(object):
  ...
  def plot_pLDDT(self, size_in_inches=12, axis_label_increment=100):
    ...
    plt.title(
      f"pLDDT for {self.saving_filename}, ranked {ranking.index(str(self.saving_filename))}")
    ...
    plt.savefig(
      f"{self.saving_pathname}/plddt_ranked_{ranking.index(str(self.saving_filename))}_{self.saving_filename}_pLDDT.png",
      dpi=300)
    # plt.show()
  ...
  def plot_PAE(self, size_in_inches=12, axis_label_increment=100):
    ...
    plt.title(
      f"PAE for {self.saving_filename}, ranked {ranking.index(str(self.saving_filename))}")
    plt.xticks(ticks, fontname="Helvetica")
    ...
    # Save plot
    plt.savefig(
      f"{self.saving_pathname}/pae_ranked_{ranking.index(str(self.saving_filename))}_{self.saving_filename}_PAE.png",
      dpi=300)

    # Generate dataframe from PAE data and save to csv
    pd_PAE = pd.DataFrame(self.PAE)
    pd_PAE.to_csv(
      f"{self.saving_pathname}/pae_ranked_{ranking.index(str(self.saving_filename))}_{self.saving_filename}_PAE.csv")
...
class AlphaFoldPickle(AlphaFoldMetaData):
  ...
  def __init__(self, PathToFile, FastaSequence=None, ranking=None):
    ...
    if str(self.PathToFile).endswith("pkl"):
      with open("{}".format(self.PathToFile), "rb") as openfile:
        while True:
          try:
            ...
            break
    elif str(self.PathToFile).endswith("pkl.gz"):
      with gzip.open(self.PathToFile, 'rb') as openfile:
        while True:
          try:
            self.data.append(pkl.load(openfile))
          except EOFError:
...
if __name__ == "__main__":
  ...
  if os.path.exists(f"{cwd}/ranking_debug.json"):
    ranking_record = pathlib.Path(f"{cwd}/ranking_debug.json").resolve()
    ranking = json.load(open(ranking_record, 'r'))['order']
  elif len(glob.glob(f"{cwd}/*_plddt.json")) != 0:
    # process Unifold results
    ranking_dict=json.load(open(glob.glob(f"{cwd}/*_plddt.json")[0],'r'),)
    ranking_list=sorted(ranking_dict.items(), key=lambda d:d[1], reverse = True)
    ranking=[x[0] for x in ranking_list]
  ...
  PAEfiles = [pathlib.Path(i).resolve() for i in list(glob.glob(f'{cwd}/*.pkl*'))
              if "features.pkl" not in i and "feature.pkl.gz" not in i and "uniprot.pkl.gz" not in i ]
  #print(PAEfiles)
  # print("Select pLDDT files for upload")
  pLDDTfiles = PAEfiles.copy()
  #print(pLDDTfiles)
  ...
  for PAEfile in PAEfiles:
    ...
    if PAEfile.suffix == ".pkl" or PAEfile.suffix == ".gz":
      results = AlphaFoldPickle(PAEfile, None)
      results.saving_pathname = save_dir
      results.saving_filename = PAEfile.stem if PAEfile.suffix != ".gz" else PAEfile.name.replace("_outputs.pkl.gz",'')
      # process pae
  ...
  for pLDDTfile in pLDDTfiles:
    ...
    if fileType == ".pkl" or fileType == ".gz" :
      results = AlphaFoldPickle(pLDDTfile, None)
      results.saving_filename = fileName if fileType != ".gz" else pLDDTfile.name.replace("_outputs.pkl.gz",'')
      results.saving_pathname = save_dir
